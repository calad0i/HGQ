...
def floatlet_quantize(x, m, e, e0=0):
    ...
    e_req = tf.math.floor(tf.math.log(tf.abs(x) + eps) / log2) + 1  # type: ignore
    e_low, e_high = -two**(e - 1) + e0, two**(e - 1) - 1 + e0
    e_act = tf.clip_by_value(e_req, e_low, e_high)
    scale = two**(e_act - m + 1)
    sig = x / scale
    qsig = tf.floor(sig)
    clip_sig = tf.clip_by_value(qsig, -two**(m - 1), two**(m - 1) - 1)
    qx = clip_sig * scale
...
def floatlet_decompose(x, m, e, e0=0):
    ...
    sig = x / scale
    qsig = np.floor(sig)
    clip_sig = np.clip(qsig, -2.**(m - 1), 2.**(m - 1) - 1) / 2**(m - 1)
