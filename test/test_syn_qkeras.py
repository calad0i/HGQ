...
def create_model():
    ...
    l = lambda X: qkeras.QDense(X, kernel_quantizer=quantizers.quantized_bits(8, 3, alpha=1.), activation=quantizers.quantized_bits(5, 3, alpha=1.), bias_quantizer=quantizers.quantized_bits(8, 3, alpha=1))
    c = lambda X: qkeras.QConv2D(X, 2, kernel_quantizer=quantizers.quantized_bits(8, 3, alpha=1.), activation=quantizers.quantized_bits(5, 3, alpha=1.), bias_quantizer=quantizers.quantized_bits(8, 3, alpha=1))
